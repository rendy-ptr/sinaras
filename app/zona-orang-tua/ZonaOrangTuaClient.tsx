...
import { Clock, Smile, MessageCircle, Trophy, Sparkles, Heart, Star, Sparkle } from 'lucide-react';
...
export interface ZonaOrangTuaTip {
  title: string;
  description: string;
  iconName: string;
  colorTheme: string;
...
export interface ZonaOrangTuaData {
  _id: string;
  title: string;
  subTitle: string;
  ...
  tipsTitle: string;
  tipsSubtitle?: string;
  tips?: ZonaOrangTuaTip[];
}

interface Props {
  data: ZonaOrangTuaData | null;
}
...
export default function ZonaOrangTuaClient({ data }: Props) {
  if (!data) {
    return (
        ...
        <main className="relative z-10 max-w-6xl mx-auto px-4 py-32 flex-grow flex items-center justify-center w-full">
          <div className="bg-white border-4 border-slate-800 rounded-3xl p-8 md:p-12 shadow-[8px_8px_0px_0px_#1e293b] text-center max-w-lg">
            <h1 className="text-2xl font-display font-bold text-slate-800 mb-2">
              Data &ldquo;Zona Orang Tua&rdquo; Kosong
            </h1>
            <p className="text-slate-500 font-sans text-sm leading-relaxed mb-6">
              ...
              publikasikan dokumen <strong>&ldquo;IV. Zona Orang Tua&rdquo;</strong> agar konten
              muncul di sini.
            </p>
          </div>
        </main>
            ...
            {data.image && (
              ...
              >
                <Image
                  ...
                  alt={data.alt || 'Gambar Zona Orang Tua'}
                  width={500}
                  height={500}
          ...
          <div className="mb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {data.tips.map((tip, idx) => {
                const Icon = ICON_MAP[tip.iconName] || Smile;
                const colorClass = COLOR_MAP[tip.colorTheme] || COLOR_MAP.pink;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="pt-4 border-t border-slate-100 flex items-center gap-1.5 text-xs text-brand-purple font-display font-semibold">
                      <Sparkle className="w-3.5 h-3.5 fill-brand-purple" /> Tips Keluarga
                    </div>
                  </motion.div>
                );
              })}

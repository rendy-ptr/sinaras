...
import { Camera, Calendar, X, Maximize2, Download, Search, Loader2 } from 'lucide-react';
import NavbarSection from '@/components/NavbarSection';
import FooterSection from '@/components/FooterSection';
import Image from 'next/image';
import { DocumentationsItem } from './data';
...
interface Props {
  documentations: DocumentationsItem[];
  initialSearch: string;
}

export default function DocumentationsClient({ documentations, initialSearch }: Props) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [selectedDoc, setSelectedDoc] = useState<DocumentationsItem | null>(null);
  ...
  const updateParams = useCallback(
    (key: string, value: string) => {
      ...
      startTransition(() => {
        router.push(`/documentations?${params.toString()}`);
      });
    },
    [searchParams, router],
  );
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 py-12 md:py-20 flex-grow w-full">
        ...
        {isPending ? (
          <div className="flex flex-col items-center justify-center py-20 min-h-[300px]">
            ...
            <p className="mt-4 font-display text-sm font-bold text-slate-500">Memuat data...</p>
          </div>
            ...
            {documentations.map((doc) => (
              ...
              >
                <div className="aspect-video relative overflow-hidden border-b-3 border-slate-800 bg-slate-100">
                  {doc.image ? (
                    <Image
                      src={urlFor(doc.image).width(600).height(400).auto('format').quality(80).url()}
                      alt={doc.title}
                      fill
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="aspect-video relative overflow-hidden bg-slate-100 border-b-3 border-slate-800">
                {selectedDoc.image ? (
                  <Image
                    src={urlFor(selectedDoc.image).width(600).height(400).auto('format').quality(80).url()}
              ...
              <div className="overflow-y-auto p-6 text-left">
                ...
                {selectedDoc.body && (
                  <div className="prose prose-slate max-w-none font-sans text-sm md:text-base text-slate-600 leading-relaxed">
                    <PortableText value={selectedDoc.body} />
                  </div>
                )}
              </div>
              ...
              <div className="p-4 bg-slate-50 border-t-2 border-slate-200 flex justify-end items-center gap-2">
                {selectedDoc.image && (
                  <a
                    href={`${urlFor(selectedDoc.image).url()}?dl=sinaras-dokumentasi-${selectedDoc._id}.jpg`}

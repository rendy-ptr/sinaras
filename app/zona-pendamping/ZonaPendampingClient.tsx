...
export interface ZonaPendampingStrategy {
  title: string;
  description: string;
  iconName: string;
  colorTheme: string;
...
export interface ZonaPendampingUseCase {
  title: string;
  description: string;
}

export interface ZonaPendampingData {
  _id: string;
  title: string;
  subTitle: string;
  ...
  strategies?: ZonaPendampingStrategy[];
  useCasesTitle: string;
  useCasesSubtitle?: string;
  useCasesNote?: string;
  useCases?: ZonaPendampingUseCase[];
}

interface Props {
  data: ZonaPendampingData | null;
}
...
export default function ZonaPendampingClient({ data }: Props) {
  if (!data) {
    return (
        ...
        <main className="relative z-10 max-w-6xl mx-auto px-4 py-32 flex-grow flex items-center justify-center w-full">
          <div className="bg-white border-4 border-slate-800 rounded-3xl p-8 md:p-12 shadow-[8px_8px_0px_0px_#1e293b] text-center max-w-lg">
            <h1 className="text-2xl font-display font-bold text-slate-800 mb-2">
              Data &ldquo;Zona Pendamping&rdquo; Kosong
            </h1>
            <p className="text-slate-500 font-sans text-sm leading-relaxed mb-6">
              ...
              publikasikan dokumen <strong>&ldquo;III. Zona Pendamping&rdquo;</strong> agar konten
              muncul di sini.
            </p>
          </div>
        </main>
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 py-12 md:py-20 flex-grow">
            ...
            {data.image && (
              ...
              >
                <Image
                  ...
                  alt={data.alt || 'Gambar Zona Pendamping'}
                  width={500}
                  height={500}
          ...
          <div className="mb-20">
            <div className="text-center max-w-2xl mx-auto mb-12">
              <h3 className="text-2xl md:text-3xl font-display font-bold text-slate-800 tracking-tight">
                2. {data.strategiesTitle}
              </h3>
              {data.strategiesSubtitle && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {data.strategies.map((strategy, idx) => {
                const Icon = ICON_MAP[strategy.iconName] || Smile;
                const colorClass = COLOR_MAP[strategy.colorTheme] || COLOR_MAP.pink;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch mb-20">
          <div className="lg:col-span-5 flex flex-col justify-center text-left">
            ...
            <h3 className="text-2xl md:text-3xl font-display font-bold text-slate-800 tracking-tight">
              3. {data.useCasesTitle}
            </h3>
            {data.useCasesSubtitle && (
